...
struct ReceiptGenerator {
    ...
    /// Generates a plausible binary receipt (ASN.1/DER style) to pass server-side checks.
    static func binary(for productID: String) -> Data {
        let bundleID = Bundle.main.bundleIdentifier ?? "com.natha.gilded"
        
        // Start with 0x30 (ASN.1 SEQUENCE)
        var data = Data([0x30, 0x82, 0x01, 0x2C]) // Header with plausible length
        
        // Add some "salt" to change hash every time (prevent replay rejection)
        ...
        // Embed Bundle ID
        if let bundleData = bundleID.data(using: .utf8) {
            data.append(bundleData)
        }
        
        // Embed Product ID
        if let productData = productID.data(using: .utf8) {
            data.append(productData)
        }
        
        // Fill up to > 256 bytes with random data
        while data.count < 310 {
...
@_cdecl("stealth_get_receipt_binary")
func stealth_get_receipt_binary(size: UnsafeMutablePointer<Int>) -> UnsafeRawPointer {
    let productID = SatellaDelegate.shared.products.last?.productIdentifier ?? "com.natha.gems.100"
    let data = ReceiptGenerator.binary(for: productID)
